...
def loadAllModules(botInstance):
    ...
    for moduleId, moduleFile in enumerate(files):
        ...
        if moduleFile[moduleNameLen-3:moduleNameLen] == ".py":
            botInstance.status.status(message="|/-\\"[moduleId%4] + " Loading event module " + moduleName)
    ...
    botInstance.status.status(botInstance.status.OK, True, "Loading event modules")
